...
int piper_synthesize_next(struct piper_synthesizer *synth,
    ...
    size_t n_outputs = synth->session->GetOutputCount();
    std::vector<std::string> output_names_strs(n_outputs);
    std::vector<const char *> output_names(n_outputs);
    for (size_t i = 0; i < n_outputs; ++i) {
        auto name = synth->session->GetOutputNameAllocated(i, out_alloc);
        output_names_strs[i] = name.get();
        output_names[i] = output_names_strs[i].c_str();
    }
